...
r.get("/theme.css", (req, res) => {
  ...
  const root = Object.entries(vars)
    .map(([k, v]) => `${k}:${v}`)
    .join(";");
  // crmv1.46 — tek tema motoru: eski !important kural bloğu buradaydı.
  // Kaldırıldı; hiçbir yere gönderilmiyordu ama her /settings/theme.css
  // isteğinde ~30 satırlık CSS boşuna üretiliyordu. Görünümün tamamı
  // arteva-ds-v2.css'ten gelir, bu uç nokta yalnızca token yayımlar.
