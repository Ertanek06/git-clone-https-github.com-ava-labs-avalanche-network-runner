(function(){
  ...
  if(progress){
    ...
    function poll(){
      fetch("/products/import-web/scan/"+encodeURIComponent(jobId)+"/status",{headers:{accept:"application/json"},credentials:"same-origin"}).then(function(r){return r.json().then(function(j){if(!r.ok||j.ok===false)throw new Error(j.message||"Durum alınamadı");return j})}).then(function(data){
        ...
        var err=byId("webScanError"),ready=byId("webScanReady"),cancel=byId("webScanCancelForm");if(cancel)cancel.hidden=!["QUEUED","RUNNING"].includes(j.status);
  ...
  function saveField(el){var row=el.closest("tr"),index=(row?.dataset.detailIndex||row?.dataset.rowIndex||"");if(index==="")return;var field=el.dataset.webField;if(!field)return;var value=el.type==="checkbox"?(el.checked?"1":"0"):el.value;state(index,"Kaydediliyor…",false);saving++;jsonPost("/products/import-web/scan/"+encodeURIComponent(previewJobId)+"/row/"+encodeURIComponent(index),{[field]:value}).then(function(r){state(index,r.status==="UPDATE"?"✓ Mevcut ürün güncelleme için hazır":r.status==="CURRENT"?"✓ CRM ile güncel":"✓ Kaydedildi",false);var box=mainRow(index)?.querySelector(".web-row-selected");if(box){box.disabled=!r.selectable;if(!r.selectable)box.checked=false}}).catch(function(e){state(index,e.message||"Kayıt hatası",true)}).finally(function(){saving=Math.max(0,saving-1)})}
  ...
  var importForm=document.querySelector("form.web-import-dock");if(importForm)importForm.addEventListener("submit",function(e){var btn=importForm.querySelector('button[type="submit"]');if(btn){btn.disabled=true;btn.textContent="Aktarım kuyruğa alınıyor…"}if(saving<=0)return;e.preventDefault();var wait=setInterval(function(){if(saving>0)return;clearInterval(wait);HTMLFormElement.prototype.submit.call(importForm)},120)});
  preview.querySelectorAll("[data-web-bulk-select]").forEach(function(button){button.addEventListener("click",function(){
    ...
    jsonPost("/products/import-web/scan/"+encodeURIComponent(previewJobId)+"/rows/select",{indices:indices.join(","),scope:scope,q:preview.dataset.query||"",filter:preview.dataset.filter||"ALL",selected:selected?"1":"0"}).then(function(r){
      ...
      if(byId("webSelectedTotal"))byId("webSelectedTotal").textContent=String(r.selected||0);
      centerToast((selected?"Seçim yapıldı: ":"Seçim kaldırıldı: ")+Number(r.changed||0)+" ürün.","success");
    }).catch(function(e){centerToast(e.message||"Seçim kaydedilemedi.","error")}).finally(function(){button.disabled=false;saving=Math.max(0,saving-1)})
  })});
